...
use crate::providers::{ProviderRuntime, ProviderService};
...
use modula_platform::{self as platform, SpawnIo};
        ...
        Err(e) => {
            ...
            events
                .publish(
                    &ws_id,
                    events::RUN_EXITED,
                    serde_json::json!({"run_id": run_id, "status": STATUS_FAILED}),
                )
                .await;
            Err(e)
        }
...
async fn compute_spec_dir(
    ...
    let slug = crate::workspaces::task_spec_slug(task.external_id.as_deref(), &task.title);
                ...
                Err(e) => {
                    ...
                    events
                        .publish(
                            ws_id,
                            events::RUN_EXITED,
                            serde_json::json!({"run_id": next_run_id, "status": STATUS_FAILED}),
                        )
                        .await;
                    break;
                }
