...
//! those paths publish `task.update`; creating one publishes `label.create` so
//! a sync replica can resolve a task's `label_ids`.
...
impl LabelService {
    ...
    pub async fn create(&self, ws: &str, kind: &str, name: &str) -> Result<String> {
        ...
        tx.commit().await?;
        self.events
            .publish(ws, events::LABEL_CREATE, json!({ "label_id": id }))
            .await;
        Ok(id)
    }
...
mod tests {
    ...
    fn service(env: &crate::testkit::Env) -> LabelService {
