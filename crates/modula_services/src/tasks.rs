...
impl TaskService {
    ...
    pub async fn create_variants(
        ...
        // One unit of work: the existence check, the batch insert and the event
        // row commit together.
        ...
        let pending = self
            ...
            .record(
                &mut tx,
                ws,
                events::VARIANT_CREATE,
                json!({ "task_id": task_id, "variant_ids": ids }),
            )
            .await;
        tx.commit().await?;
        self.events.emit(ws, pending).await;
        Ok(created)
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn create_variants_emits_one_event_naming_the_new_ids() {
        let env = env().await;
        let svc = service(&env).await;
        let (id, _) = svc
            .create_internal(&env.ws, internal("t", None, None))
        ...
        // The failing lookup aborts the transaction before anything is recorded.
        assert!(svc.create_variants(&env.ws, "nope", 2).await.is_err());
        assert!(env.sink.types().is_empty());

        let created = svc.create_variants(&env.ws, &id, 2).await.unwrap();
        assert_eq!(env.sink.types(), vec![events::VARIANT_CREATE]);
        let (_, _, data) = env.sink.last().unwrap();
        assert_eq!(data["task_id"], json!(id));
        let ids: Vec<&str> = created.iter().map(|(id, _)| id.as_str()).collect();
        assert_eq!(data["variant_ids"], json!(ids));
    }
