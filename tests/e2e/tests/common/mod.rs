// Shared helpers used across tests/tests/*.rs.
//
// Cargo compiles each file under tests/ as a separate integration binary, so
// any helper a given binary doesn't reference shows up as dead-code there.
#![allow(dead_code)]
