//! E2E: opencode-typed provider spawns correctly and the run reaches completed,
//! over gRPC IPC.
...
#[tokio::test]
async fn opencode_provider_agent_completes() -> Result<()> {
    let h = Harness::start().await?;
    let ws = common::fresh_workspace(&h, "demo").await?;

    let cfg_dir = h.modula_dir.join("fake-opencode");
    std::fs::create_dir_all(&cfg_dir)?;
    let cfg_str = cfg_dir.to_string_lossy().to_string();

    // Create an opencode-typed provider.
    let oc_id = h
        .providers()
    ...
    let detail = h
        ...
        .get(GetProviderRequest {
            workspace_id: ws.clone(),
            provider_id: oc_id.clone(),
        })
        .await?
        .into_inner();
    assert_eq!(detail.r#type, "opencode");
    assert_eq!(detail.config_dir, cfg_str);

    // Create a manual agent using the opencode provider.
    let agent_id = h
        .agents()
        .create(CreateAgentRequest {
            ...
            rules: vec![],
            args: vec![],
            prompt: "hello from opencode".into(),
    ...
    // whether invoked as "claude" or "opencode" — no custom recipe.
