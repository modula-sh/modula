...
use crate::services::conversations::{self, ConvStream, WireEvent};
...
impl ConversationService for ConversationHandler {
    ...
    async fn attach(
        ...
        let body = req.into_inner();
        let handle =
            conversations::open_attach(self.state.clone(), body.workspace_id, body.conversation_id)
                .await;
        Ok(Response::new(into_stream(handle)))
    }
    ...
    async fn cancel(
        ...
        let body = req.into_inner();
        conversations::cancel(self.state.clone(), body.workspace_id, body.conversation_id)
            .await
            .map_err(to_status)?;
